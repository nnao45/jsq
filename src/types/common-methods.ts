...
export interface ArrayMethods<T> {
  ...
  orderBy<K extends keyof T>(keys: K | K[] | ((item: T) => any) | ((item: T) => any)[], orders?: ('asc' | 'desc') | ('asc' | 'desc')[]): any;
...
export interface LodashMethods<T> extends ArrayMethods<T>, ObjectMethods<T>, StringMethods, UtilityMethods {
  // lodash特有のメソッド
  chain<U>(value: U): any;
