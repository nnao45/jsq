...
export class EnquirerAutocompleteAdapter {
   ...
   * 補完を適用した後の値を取得
   */
  private getCompletionValue(completion: string, result: CompletionResult, input: string): string {
   ...
   * リアルタイム検索用のフィルター
   */
  filterSuggestions(suggestions: EnquirerSuggestion[], query: string): EnquirerSuggestion[] {
