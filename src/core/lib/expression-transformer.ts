...
export function transformExpression(
  ...
  // Handle semicolon operations (sequential execution)
    ...
    // Debug logging removed to avoid interfering with output
...
/**
 * Check if expression uses semicolon as a separator between independent expressions
 * (not just as statement terminator within a code block)
 */
function isUsingSemicolonAsSeparator(expression: string): boolean {
  ...
  if (expression.includes('\n')) {
    ...
    const hasMultipleExpressionsPerLine = lines.some(line => {
