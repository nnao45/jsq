import type { MethodImplementation, SmartDollarMethods } from '../types/common-methods';
...
const commonMethods = {
  ...
  reduce<U>(
    data: unknown[],
    reducer: (acc: U, value: unknown, index: number, array: unknown[]) => U,
    initial?: U
  ) {
  ...
  orderBy(
    data: unknown[],
    keys: string | string[] | ((item: unknown) => unknown) | ((item: unknown) => unknown)[],
  ...
  min(data: unknown[]) {
    return data.length > 0
      ? Math.min(...(data.filter(v => typeof v === 'number') as number[]))
      : undefined;
  },

  max(data: unknown[]) {
    return data.length > 0
      ? Math.max(...(data.filter(v => typeof v === 'number') as number[]))
      : undefined;
  },
  ...
  minBy(data: unknown[], key: string | ((item: unknown) => unknown)) {
    ...
    return data.reduce((min, item) => {
      const minVal =
        typeof key === 'function'
          ? key(min)
          : typeof min === 'object' && min !== null
            ? (min as Record<string, unknown>)[key]
            : undefined;
      const itemVal =
        typeof key === 'function'
          ? key(item)
      ...
      const maxVal =
        typeof key === 'function'
          ...
          : typeof max === 'object' && max !== null
            ? (max as Record<string, unknown>)[key]
            : undefined;
      const itemVal =
        typeof key === 'function'
          ? key(item)
...
export class ChainableWrapper implements MethodImplementation<SmartDollarMethods<unknown>> {
  ...
  // 非同期メソッド
  async mapAsync<U>(
    mapper: (value: unknown, index: number, array: unknown[]) => Promise<U>
  ...
  async forEachAsync(
    callback: (value: unknown, index: number, array: unknown[]) => Promise<void>
  ): Promise<void> {
    if (this.isArray()) {
