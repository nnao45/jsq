...
describe.skip('REPL Autocomplete', () => {
  const createTestSetup = () => {
    ...
    const testData = {
      ...
      userCount: 2,
      userList: ['Alice', 'Bob'],
      userMap: new Map([['alice', 1], ['bob', 2]]),
    };
    
    const mockEvaluator: EvaluationHandler = vi.fn(async (expression, data, opts) => {
      ...
      if (opts?.isAutocomplete) {
        ...
        if (prefix === '') {
          ...
          suggestions.push('config.database.host', 'config.database.port', 'config.database.username');
        }
  ...
  describe('Tab Completion', () => {
    ...
    it('should autocomplete single match', async () => {
      ...
      mockEvaluator.mockImplementation(async (expression, data, opts) => {
  ...
  describe('Edge Cases', () => {
    it('should handle no suggestions gracefully', async () => {
      ...
      await mockInput.playAll(0, inputBefore.split('').map(c => ({ str: c })));
