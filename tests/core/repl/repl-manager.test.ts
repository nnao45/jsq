...
describe('ReplManager', () => {
  const createTestSetup = () => {
    ...
    const mockEvaluator: EvaluationHandler = vi.fn(async (expression) => {
      if (expression === '.') {
  ...
  describe('Character Input', () => {
    ...
    it('should handle enter key and evaluate expression', async () => {
      ...
      expect(mockEvaluator).toHaveBeenCalledWith('.', { test: 'data' }, expect.any(Object));
  ...
  describe('Real-time Evaluation', () => {
    it('should perform real-time evaluation when enabled', async () => {
      ...
      // Wait for debounce delay (300ms default) plus a small buffer
  ...
  describe('Complex Expression Evaluation', () => {
    it('should evaluate complex object paths', async () => {
      ...
      expect(evaluator).toHaveBeenCalledWith('.users[0].details.city', complexData, expect.any(Object));
    });
  ...
  describe('Special Characters and Unicode', () => {
    ...
    it('should handle special characters in expressions', async () => {
      ...
      expect(specialEvaluator).toHaveBeenCalledWith('.prop["key with spaces"]', { prop: { 'key with spaces': 'value' } }, expect.any(Object));
    });
  });
  ...
  describe('Edge Cases and Stress Tests', () => {
    it('should handle rapid key inputs', async () => {
      ...
      await mockInput.playAll(0, chars.map(c => ({ str: c })));
